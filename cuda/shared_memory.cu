...
__global__ void warmup(int *out) {
    ...
    /**
     * 同步有两种方式 refer to:https://face2ai.com/CUDA-F-5-1-CUDA%E5%85%B1%E4%BA%AB%E5%86%85%E5%AD%98%E6%A6%82%E8%BF%B0/
     * 1. barrier
     * 2. 内存栅栏
     *
     * 这里采用 barrier
     *  __syncthreads()
     * 保证在同一线程块内所有线程没到达此障碍点时，不能继续向下执行
     * 想做块之间的同步，只能通过核函数的执行和结束来进行块之间的同步
     */
    __syncthreads();
